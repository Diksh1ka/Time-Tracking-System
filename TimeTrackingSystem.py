...
def create():
    print("1.create employee 2.create project")
    ...
    if(ch==1):
        ...
        emp_info={'name':nm,'age':em_age,'mail':mail_id,'pno':ph_no}
    ...
    if(ch==1):
        if(len(emp_details)==0):
            ...
            for i in emp_details:
                print("%s\t\t%d\t\t%s\t\t%s"%(emp_details[i]['name'],emp_details[i]['age'],emp_details[i]['mail'],emp_details[i]['pno']))
    elif(ch==2):
        if(len(prjt_dtls)==0):
...
def generate_time_summary_report():
    prjt_id = int(input("Enter project id : "))
    if prjt_id in prjt_dtls:
        assigned_emp_id = int(input("Enter employee id assigned to this project: "))
        if assigned_emp_id in emp_details:
            login=float(input("enter log_in time : "))
            logout=float(input("enter log_out time : "))
            total_hrs=logout-login
            extra_hrs=total_hrs-6
            remaining_hrs=(prjt_dtls[prjt_id][dur])-total_hrs
            if(total_hrs==6):
                price=total_hrs*100
            elif(total_hrs<=5):
                price=total_hrs*80
                ...
                price=6*100+(extra_hrs)*200
            print(f"Employee ID: {assigned_emp_id}")
            print(f"Remaining Hours: {remaining_hrs} hours")
            print(f"Total Working Hours: {total_hrs} hours")
            print(f"Price for Working Hours: ${price}")
            print(f"Remaining Hours: {remaining_hrs} hours")
        else:
...
while True:
    print('1.create 2.display 3.update 4.delete 5.calculate_billable_hrs 6.generate_time_summary_report')
